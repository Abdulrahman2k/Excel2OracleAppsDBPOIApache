SET DEFINE OFF;
CREATE OR REPLACE package      XXTGPL_AME_APPROVAL_PKG is 
 PROCEDURE INITIATE_XXTGPL_WORKFLOW (itemtype        in varchar2,
                            itemkey         in varchar2,
                            actid           in number,
   ...
   procedure updateAmeWithResponse(itemtype        in varchar2,
      ...
      procedure getnextFYINotifiers(  itemtype        in varchar2,
                                 ...
                                 funcmode        in varchar2,
                                 resultout       out NOCOPY varchar2);
      PROCEDURE GET_WF_ADD_APPR_HTML_URL ( document_id IN VARCHAR2
                                ,display_type IN VARCHAR2
   ...
   FUNCTION GET_ACCOMM_MANAGER(P_TRANSACTION_ID IN NUMBER) RETURN VARCHAR2;
END XXTGPL_AME_APPROVAL_PKG;
/


CREATE OR REPLACE package body      XXTGPL_AME_APPROVAL_PKG is 
         ...
         from PER_SECURITY_PROFILES_V 
          where SECURITY_PROFILE_ID = FND_PROFILE.VALUE('PER_SECURITY_PROFILE_ID');
            CURSOR XXTG_ADDRESS_CHANGE_CUR (LN_TRANACTION_ID NUMBER)
                          ...
                          AND XCLA.CAMP_NAME IS NOT NULL
                          AND 
            ...
            CURSOR XXTG_DISTINCT_ACCOMM_CUR (LN_TRANSACTION_ID NUMBER)
            IS 
            SELECT  listagg (CAMP_NAME ,'--') WITHIN GROUP (ORDER BY CAMP_NAME)  ACCOMMODATION --listagg ("ID" || ' ' || "Name", ' ') WITHIN GROUP (ORDER BY ID) Result
            FROM
            (
            SELECT  DISTINCT Trim(XCLA.CAMP_NAME)  CAMP_NAME
                          FROM  PER_ADDRESSES pa, XXTG_CA_LINES_ALL XCLA
                          where pa.person_Id =XCLA.PERSON_ID
                          AND XCLA.EFFECTIVE_DATE between pa.date_from and nvl(pa.date_to,fnd_conc_date.string_to_date('31-Mar-3172'))
                          AND XCLA.CAMP_NAME IS NOT NULL
                          ...
                          TRIM(UPPER( hr_general.DECODE_FND_COMM_LOOKUP (p_lookup_type =>'TG_CAMP_NAME',
                                                        p_lookup_code=>pa.add_information20 ))) <>  TRIM(UPPER( XCLA.CAMP_NAME))
                          AND XCLA.HEADER_ID = LN_TRANSACTION_ID
              );
                          LN_COUNT_EMP_WITH_CHANGE_ADD NUMBER;
                          LC_ACCOMMODATION_NAME VARCHAR2(1000):= NULL;
BEGIN
 --Assigning values to Variables---
        l_transaction_id:= TO_NUMBER(wf_engine.getItemAttrText( itemtype =>  itemtype,
                                                         itemkey  => itemkey,
                                                         aname    => 'HEADER_ID' ));
 x_itemtype:= 'XXTGPL'; -- This should be internal name of workflow
 x_itemkey:= TO_CHAR(l_transaction_id);
 x_process:= 'XXTG_CA_ADDRESS_UPDATE'; ---Workflow Process Internal name
--        p_requester : = fnd_profile.VALUE('USERNAME');
--        p_mail_id: = 'xyz@abc.com'; 
--        p_vendor_name: = 'AD Infotech Pvt Ltd';

           --First we create the process--- 
      ...
      wf_engine.setitemattrtext( itemtype =>  x_itemtype,
            ...
            LN_COUNT_EMP_WITH_CHANGE_ADD:=0;
FOR XXTG_CA_REC IN  XXTG_ADDRESS_CHANGE_CUR (l_transaction_id)
  LOOP
    LN_COUNT_EMP_WITH_CHANGE_ADD:= XXTG_CA_REC.count;
  END LOOP;
  IF LN_COUNT_EMP_WITH_CHANGE_ADD >0 THEN
             wf_engine.setitemattrtext(itemtype => x_itemtype,
                                itemkey => x_itemkey,
                                aname => 'XXTG_COUNT_EMP_WITH_CHANGE',
                                avalue => LN_COUNT_EMP_WITH_CHANGE_ADD
                                );
  END IF;
  LC_ACCOMMODATION_NAME := NULL;
  FOR XXTG_ACCOMM_REC IN XXTG_DISTINCT_ACCOMM_CUR(l_transaction_id)
    LOOP
      LC_ACCOMMODATION_NAME:= XXTG_ACCOMM_REC.ACCOMMODATION;
    END LOOP;
    IF LC_ACCOMMODATION_NAME IS NOT NULL THEN
             wf_engine.setitemattrtext(itemtype => x_itemtype,
                                itemkey => x_itemkey,
                                aname => 'XXTG_ACCOMMODATION_NAME',
                                avalue => LC_ACCOMMODATION_NAME
                                );
    END IF;
             wf_engine.setitemattrtext(itemtype => x_itemtype,
                                itemkey => x_itemkey,
      ...
      LC_TOP_BU_NAME:= NULL;                         
       FOR  XXTG_SEC_REC IN XXTG_SECURITY_PROFILE
         ...
         END LOOP;
            ...
            SET ADDRESS_APPROVAL_STATUS ='In Approval Process',
            CHANGE_IN_ADD_FLAG ='Y',
...
--          WF_ITEM_KEY        = x_itemkey,
            ...
            avalue => 'PLSQL:XXTGPL_AME_APPROVAL_PKG.GET_WF_HTML_URL/' || x_itemkey);
            
            wf_engine.SetItemAttrText (itemtype => x_itemtype,
            itemkey =>x_itemkey,
            aname =>'XXTG_APP_ADD_BODY_WITH_LINK',
            avalue => 'PLSQL:XXTGPL_AME_APPROVAL_PKG.GET_WF_ADD_APPR_HTML_URL/' || x_itemkey);
...
--            AME_TRANSACTION_TYPE ='XXTGCA',
            ...
            itemkey =>itemkey,
            aname =>'XXTG_BODY_WITH_LINK',
            avalue => 'PLSQL:XXTGPL_AME_APPROVAL_PKG.GET_WF_HTML_URL/' || itemkey);
              wf_engine.SetItemAttrText (itemtype => itemtype,
            itemkey =>itemkey,
            aname =>'XXTG_APP_ADD_BODY_WITH_LINK',
            avalue => 'PLSQL:XXTGPL_AME_APPROVAL_PKG.GET_WF_ADD_APPR_HTML_URL/' || itemkey);
  ...
  ( P_ORGANIZATION =>  XXTG_REC.NEW_CONTRACT_NAME,  -- VARCHAR2,
    ...
    END LOOP;
        UPDATE  XXTG_CA_HEADERS_ALL
        SET ADDRESS_APPROVAL_STATUS ='Approved  Sent',
        CHANGE_IN_ADD_APPROVAL_FLAG='Y',
        CHANGE_IN_ADD_FLAG ='Y',
        ADDRESS_APPROVED_DATE = SYSDATE,
--        CLOSED_DATE = SYSDATE,
--        APPROVAL_REQUIRED_FLAG ='N',
--        AME_TRANSACTION_TYPE ='XXTGPL',
        AME_APPROVAL_ID = l_transaction_id
        WHERE HEADER_ID = l_transaction_id;
...
UPDATE xxtg_ca_headers_all 
SET ADDRESS_APPROVAL_STATUS ='Rejected',
CHANGE_IN_ADD_FLAG ='Y',
CHANGE_IN_ADD_APPROVAL_FLAG='N',
APPROVAL_REQUIRED_FLAG ='Y',
ADDRESS_APPROVED_DATE = SYSDATE,
--AME_TRANSACTION_TYPE ='XXTGPL',
AME_APPROVAL_ID = l_transaction_id
...
BEGIN
  --
  ln_header_id  := to_number(replace(document_id, 'PLSQL:XXTGPL_AME_APPROVAL_PKG.CREATE_WF_DOC/',''));
...
--        END LOOP;
      XXTGPL_AME_APPROVAL_PKG.getAmeTransactionDetail  ( l_transaction_type,l_application_id );
...
--        resultout:='COMPLETE:'||'N'; 
        wf_core.context('XXTGPL_AME_APPROVAL_PKG',
                        'getNextApprover',
                        itemtype,
                        itemkey,
                        to_char(actid),
...
ln_header_id  := to_number(replace(document_id, 'PLSQL:XXTGPL_AME_APPROVAL_PKG.GET_WF_HTML_URL/',''));
document_type := 'text/html';
ln_function_id :=fnd_function.get_function_id ('XXTG_CA_ADDR_NOTIFICATION'); --('XXTG_CA_NOTIFICATION');
 ln_user_id := null;
    ln_resp_id := null;
...
) ||'"> OPEN ADDRESS APPROVAL BATCH LINK [/URL]';
--
...
ln_header_id  := to_number(replace(document_id, 'PLSQL:XXTGPL_AME_APPROVAL_PKG.GET_WF_ADD_APPR_HTML_URL/',''));
document_type := 'text/html';
ln_function_id :=fnd_function.get_function_id ('XXTG_CA_ADDR_UPDATE'); --('XXTG_CA_NOTIFICATION');
 ln_user_id := null;
    ln_resp_id := null;
    ln_resp_appl_id := null;
--   select FND_PROFILE.VALUE('USER_ID') into ln_user_id from dual;
...
--, p_parameters => P_HEADER_ID='||ln_header_id||'&nbsp;P_READ_ONLY=Y&nbsp
, p_parameters => 'P_HEADER_ID='||ln_header_id --||'&'||'P_READ_ONLY=Y'
) ||'"> OPEN ADDRESS PROCESSING BATCH LINK [/URL]';
--
l_document := l_document || '<BR> ' || l_htmlref;
document := l_document;
--insert into xxtg_test values (ln_header_id);
end GET_WF_ADD_APPR_HTML_URL; 

  FUNCTION GET_ACCOMM_MANAGER(P_TRANSACTION_ID IN NUMBER) RETURN VARCHAR2 IS
  
    L_SELECTED_PERSON_ID NUMBER;
    L_EFFECTIVE_DATE     DATE;
    --l_emp_asg_org_id     NUMBER;
    L_BUSINESS_GROUP_ID NUMBER;
    --l_approver           VARCHAR2(2000);
    --l_bu_org_id          NUMBER;
    --x3_person_id         VARCHAR2(2000);
    --bu_person_id         VARCHAR2(2000);
    --sbu_person_id        VARCHAR2(2000);
    L_CREATOR_ID NUMBER;
    L_PAYROLL_ID NUMBER;
    L_ROLE_ID    VARCHAR2(1000);
    CURSOR XXTG_ROLE (LN_TRANSACTION_ID NUMBER ) IS
    SELECT 'PQH_ROLE:'||attribute5 ROLE
FROM FND_LOOKUP_VALUES WHERE LOOKUP_TYPE ='TG_CAMP_NAME'
AND ATTRIBUTE5 IS NOT NULL AND LANGUAGE='US' AND ENABLED_FLAG ='Y'
AND UPPER(TRIM(hr_general.DECODE_LOOKUP('TG_CAMP_NAME',lookup_code))) = (  --10078
SELECT DISTINCT(UPPER(TRIM(CAMP_NAME))) FROM XXTG_CA_LINES_ALL WHERE HEADER_ID = LN_TRANSACTION_ID
AND ROWNUM = 1);
  BEGIN
      L_ROLE_ID := NULL;
     FOR XXTG_REC IN XXTG_ROLE(P_TRANSACTION_ID) 
       LOOP
         L_ROLE_ID := XXTG_REC.ROLE;
       END LOOP;
     
     IF L_ROLE_ID IS NULL THEN
     L_ROLE_ID := 'PQH_ROLE:50032';
     END IF;
    RETURN L_ROLE_ID;
  END GET_ACCOMM_MANAGER;
END XXTGPL_AME_APPROVAL_PKG;
/
